  ...
  >
       ...
       <div className="flex flex-row justify-between items-start mb-12 gap-4">
           <div className="w-2/5">
               <div data-export-id="logo-container">
                 {data.logoUrl ? (
                     <div className="relative group">
                         ...
                         <button onClick={() => removeImage('logoUrl')} className="absolute top-0 left-0 bg-red-500 text-[#ffffff] rounded-full h-6 w-6 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity preview-only">&times;</button>
                     </div>
                 ) : (
               ...
               <div className="mt-4 ">
                   <EditableField
                      ...
                      className="text-lg w-[300px] text-[#ffffff] dark:text-[#ffffff] border-[#d1d5db] dark:border-[#d1d5db]"
                      rows={4}
                      
                   />
               </div>
               <div className="mt-8">
                 <h2 className="text-sm font-bold text-[#ffffff] dark:text-[#ffffff] uppercase tracking-wide mb-2">{t.billTo}</h2>
                 <EditableField
                    value={data.to}
                    onChange={handleDataChange}
                    name="to"
                    placeholder={"Client Company\n456 Client Street\nClient City, ST 54321\nclient@email.com"}
                    className="text-lg text-[#ffffff] dark:text-[#ffffff] w-[300px] border-[#d1d5db] dark:border-[#d1d5db] "
                    rows={4}
                 />
               </div>
           </div>
           ...
           <div className="w-auto text-right">
               <h1 className="text-5xl font-bold uppercase text-[#ffffff] tracking-widest ">
                   {data.documentType === DocumentType.Invoice ? t.documentTypeInvoice : t.documentTypeEstimate}
               </h1>
               <div className="flex items-center inline-flex justify-end mr-1 mt-3">
                   <span className="text-[#ffffff] mr-1 ">{t.invoiceNumberPrefix}</span>
                   <EditableField
                      value={data.invoiceNumber}
                      onChange={handleDataChange}
                      name="invoiceNumber"
                      className=" text-right mt-2 text-[#ffffff] dark:text-[#ffffff] font-semibold border-[#d1d5db] dark:border-[#d1d5db] "
                      style={{ width: '220px' }}
                   />
               </div>
               <div className="space-y-2 mt-[50px]">
                 <div className="grid grid-cols-[1fr_auto] mr-1 items-center gap-4">
                    <span className=" font-semibold text-[#ffffff] dark:text-[#ffffff] text-right ">{t.date}</span>
                    <EditableField type="date" name="date" value={data.date} onChange={handleDataChange} 
                    className=" w-[130px] text-right text-[#ffffff] dark:text-[#ffffff] max-w-40 border-[#d1d5db] dark:border-[#d1d5db]"  />
                    
                 </div>
                 <div className="grid grid-cols-[1fr_auto] mr-1 items-center gap-4">
                    <span className="font-semibold text-[#ffffff] dark:text-[#ffffff] text-right ">{t.dueDate}</span>
                    <EditableField type="date" name="dueDate" value={data.dueDate} onChange={handleDataChange} 
                    className=" w-[130px] text-right text-[#ffffff] dark:text-[#ffffff] max-w-40 border-[#d1d5db] dark:border-[#d1d5db]" />
                 </div>
                 <div data-export-id="payment-terms-row" className="grid grid-cols-[1fr_auto] mr-1 items-center gap-4">
                    <span className="font-semibold text-[#ffffff] dark:text-[#ffffff] text-right ">{t.paymentTerms}</span>
                    <EditableField type="text" name="paymentTerms" value={data.paymentTerms} onChange={handleDataChange} placeholder="e.g. Net 15" 
                    className="w-[130px] text-right text-[#ffffff] dark:text-[#ffffff] max-w-36 border-[#d1d5db] dark:border-[#d1d5db]" />
                 </div>
                 <div data-export-id="po-number-row" className="grid grid-cols-[1fr_auto] mr-1 items-center gap-4">
                    <span className="font-semibold text-[#ffffff] dark:text-[#ffffff] text-right ">{t.poNumber}</span>
        ...
        <div className="flow-root line-items-table">
          <div className="overflow-x-auto">
            <table className="min-w-full table-fixed">
                ...
                <thead>
  <tr className="text-bold text-[#ffffff] dark:text-[#ffffff]">
    ...
    <th className="py-2 px-4 text-right font-semibold align-middle">Amount</th>
                        ...
                        <tr key={item.id} className="border-b border-gray-200 group 0">
                            <td className="py-2 pl-4 pr-1 align-top">
                                <EditableField
                                    ...
                                    className=" w-[420px] text-left text-[#ffffff] dark:text-[#ffffff] border-[#d1d5db] dark:border-[#d1d5db]"/>
                            </td>
                            <td className="py-2 pl-1 pr-1 align-top text-center">
                                <EditableField type="number" value={item.quantity} onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleItemChange(item.id, 'quantity', e.target.value)}  placeholder="0"
                                  className=" w-[90px] text-center text-[#ffffff] dark:text-[#ffffff] border-[#d1d5db] dark:border-[#d1d5db]" />
                            </td>
                            <td className="py-2 pl-1 pr-3 align-top text-right">
                                ...
                                className=" w-[110px] text-right text-[#ffffff] dark:text-[#ffffff] border-[#d1d5db] dark:border-[#d1d5db]" />
                            </td>
                            <td className="py-2 pl-1 pr-0 text-right align-top text-[#ffffff] dark:text-[#ffffff] ">{currencySymbol}{formatNumber(Number(item.quantity) * Number(item.price))}</td>
      ...
      <button onClick={addItem} className="mt-4 px-4 py-2 border-2 border-dashed border-gray-300 text-[#ffffff] dark:text-[#ffffff] rounded-md hover:border-primary-500 hover:text-primary-500 transition-all preview-only">{t.addLineItem}</button>
        ...
        <div className="mt-12 flex flex-row justify-between items-start gap-8">
            <div className="w-[35%] mt-8  space-y-8">
                <div data-export-id="payment-method-container">
                    <h4 className="font-bold text-[#ffffff] dark:text-[#ffffff] mb-2 ">{t.paymentMethod}</h4>
                    <EditableField
                        name="paymentMethod"
                        value={data.paymentMethod}
                        onChange={handleDataChange}
                        rows={2}
                        className="text-[#ffffff] dark:text-[#ffffff] border-[#d1d5db] dark:border-[#d1d5db]"
                        placeholder={"PayPal Account: example@paypal.com\nBank Account: \nXXXX-XXXX-XXXX"}
                    />
                </div>
                <div data-export-id="notes-container">
                    <h4 className="font-bold text-[#ffffff] dark:text-[#ffffff] ">{t.notes}</h4>
                    <EditableField name="notes" value={data.notes} onChange={handleDataChange} rows={2} placeholder="Thanks for your business!"
                    className="mt-1 text-[#ffffff] dark:text-[#ffffff] border-[#d1d5db] dark:border-[#d1d5db]"  />
                </div>
            </div>
            ...
            <div className="w-[35%] mr-1 totals-summary">
                <dl className="space-y-2 text-[#ffffff] dark:text-[#ffffff]">
                    ...
                    <div data-export-id="discount-row" className="flex justify-between items-center">
                        ...
                        <dd className="flex items-center">
                            ...
                            className="max-w-20 text-right  text-[#ffffff] dark:text-[#ffffff] border-[#d1d5db] dark:border-[#d1d5db] flex-shrink-0" />
                            <span>%</span>
                        </dd>
                    </div>
                     <div data-export-id="tax-row" className=" text-[#ffffff] dark:text-[#ffffff] flex justify-between items-center">
                        <dt>{t.tax}</dt>
                        <dd className="flex items-center">
                            <EditableField 
                            type="number" 
                    ...
                    <div data-export-id="total-row" className="flex justify-between font-bold text-lg pt-2 border-t border-gray-300 text-[#ffffff] dark:text-[#ffffff]">
                        <dt>{t.total}</dt>
                    ...
                    <div data-export-id="amount-paid-row" className="flex justify-between items-center pt-1">
                      ...
                      <dd className="flex items-center">
                        ...
                        <EditableField
                          ...
                          onChange={handleDataChange}
                          className="text-right text-[#ffffff] dark:text-[#ffffff] border-[#d1d5db] dark:border-[#d1d5db] flex-shrink-0"
                          placeholder="0.00"
                          style={{ width: '120px' }}
                        />
                      </dd>
                    </div>
                    <div data-export-id="balance-due-row" className="flex justify-between font-bold text-xl pt-2 mt-2 border-t-2 border-gray-500 text-[#ffffff] dark:text-[#ffffff] balance-due-row">
                      <dt>{t.balanceDue}</dt>
        ...
        <div className="mt-12 flex flex-row justify-between items-end gap-8 pt-3 ">
            <div>
                {showQrCode && qrCodeUrl && (
                  <div>
                    ...
                    <p className="text-xs text-[#ffffff] dark:text-[#ffffff] mt-1 max-w-[96px] text-center ">{t.qrCodeHelper}</p>
                  </div>
                )}
            </div>
            ...
            <div data-export-id="signature-container" className="w-48 text-center">
              <div className="h-20 mb-2">
                {data.signatureUrl ? (
                  <div className="relative group h-full">
                    ...
                    <button onClick={() => removeImage('signatureUrl')} className="absolute top-0 right-0 bg-red-500 text-[#ffffff] rounded-full h-6 w-6 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity preview-only">&times;</button>
                  </div>
                ) : (
              ...
              <div className="border-t border-[#1C166C] pt-1 ">
                <p className="text-sm font-semibold text-[#ffffff] dark:text-[#ffffff] ">{t.authorizedSignature}</p>
              </div>
            </div>
        </div>
    </div>
